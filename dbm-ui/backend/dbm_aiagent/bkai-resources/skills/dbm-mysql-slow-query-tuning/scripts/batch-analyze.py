...
输出:
  $OUTPUT_DIR/slowlog_<cluster_domain>_explain.json  — EXPLAIN 结果 map (key=query_digest_md5)
  $OUTPUT_DIR/slowlog_<cluster_domain>_schema.json   — 表结构结果 map (key=query_digest_md5)
"""
...
def main():
    ...
    for i, entry in enumerate(slow_logs):
        ...
        # 即使某条异常，也要保证 explain_results/schema_results 有对应 key（空占位）
        schema_resp = {"error": "skipped", "response_body": {"data": {}}}
        explain_resp = {"error": "skipped", "response_body": {"data": {}}}
        ...
        try:
            ...
            schema_resp = {"error": str(e), "response_body": {"data": {}}}
        ...
        try:
            ...
            explain_resp = {"error": str(e), "response_body": {"data": {}}}

        # 节省一点上下文
        try:
            schema_resp.pop("request_id", None)
            schema_resp.pop("status_code", None)
            schema_resp.get("response_body", {}).pop("request_id", None)
            schema_resp.get("response_body", {}).pop("result", None)
            explain_resp.pop("request_id", None)
            explain_resp.pop("status_code", None)
            explain_resp.get("response_body", {}).pop("request_id", None)
            explain_resp.get("response_body", {}).pop("result", None)
        except Exception as e:
            pass

        schema_results[digest_md5] = schema_resp
        explain_results[digest_md5] = explain_resp
    ...
    print(f"EXPLAIN results ({len(explain_results)} entries): {explain_output}")
    print(f"Schema results ({len(schema_results)} entries): {schema_output}")
